...
bench_pairs=( [1k9k_A.pdb]=1k9p_A.pdbi)
...
avail_cpus=$(sinfo -o "%C" | tail -n1 | grep -Eo "[0-9]+" | head -n2 | tail -n1)
echo "IDLE CPUS ON CLUSTER: ${avail_cpus}"
sbatch runc.sh ${prot} ../benchmark/allosteric/pre_processed/${prot_path} ../benchmark/allosteric/pre_processed/${bench_pairs[$prot_path]} rmsd
done
